...
### 1.1**Gree.FifmSystem.EntityFrameworkCore.DbMigrations **  设置为启动项

- 通过迁移来创建数据库,需在`工具->NuGet包管理器->程序包管理器控制台`选择`执行命令所在的项目`下输入以下命令
...
![image-20210408105103866](C:\Users\180454\AppData\Roaming\Typora\typora-user-images\image-20210408105103866.png)

![image-20210408104825393](C:\Users\180454\AppData\Roaming\Typora\typora-user-images\image-20210408104825393.png)

### 1.2使用项目中**/Quartz_tables_sqlServer.sql.txt **SQL语句添加quartz
...
### 1.3Gree.FifmSystem.DbMigrator设为项目启动项并启动
...
检查**.web**项目下的**appsettings.json**文件中的**连接字符串**:
...
这个解决方案配置了**Entity Framework Core**与**MS SQL Server**一起使用. EF Core支持各种数据库
...
#### 2.2.1 使用DbMigrator应用程序应用迁移(推荐使用)

该解决方案包含一个控制台应用程序(*.DbMigrator`),可以创建数据库,应用迁移和初始化数据. 它对开发和生产环境都很有用.
...
#### 2.2.2使用EF Core Update-Database命令

Ef Core具有`Update-Database`命令, 可根据需要创建数据库并应用挂起的迁移. 右键单击`.Web`项目并选择**设置为启动项目**:

右键单击`.Web`项目并选择**设置为启动项目**:***工具->NuGet包管理器->程序包管理器控制台***
